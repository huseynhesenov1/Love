class HeartAnimation {
  constructor() {
    ...
    this.heartPoints = [];
    this.numCaplets = window.innerWidth < 600 ? 40 : 80;
  ...
  generateHeartPoints() {
    ...
    const scale =
      ...
      (window.innerWidth < 600 ? 0.09 : 0.12);
  ...
  animateCaplets() {
    this.caplets.forEach((caplet, index) => {
      setTimeout(() => {
        ...
        caplet.element.style.transform = `rotate(${
          Math.random() * 360
        }deg) scale(1.1)`;

        // Add individual caplet glow effect
        setTimeout(() => {
          caplet.element.style.filter =
  ...
  addCompletionEffects() {
    ...
    this.caplets.forEach((caplet, index) => {
      setTimeout(() => {
        caplet.element.style.filter =
          "drop-shadow(0 0 15px rgba(236, 72, 153, 1)) drop-shadow(0 0 25px rgba(168, 85, 247, 0.5))";
      }, index * 20);
    });
  }
  ...
  resetAnimation() {
    ...
    this.startBtn.innerHTML =
      '<span class="btn-icon">✨</span><span class="btn-text">Create Magic</span>';
  ...
  handleResize() {
    ...
    this.generateHeartPoints();
    this.caplets.forEach((caplet, index) => {
      if (index < this.heartPoints.length) {
...
function createSparkle(x, y) {
  ...
  sparkle.style.background =
    "radial-gradient(circle, rgba(255, 255, 255, 1) 0%, rgba(236, 72, 153, 0.8) 70%, transparent 100%)";
